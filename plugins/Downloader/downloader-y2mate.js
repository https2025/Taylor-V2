...
const SUPPORTED_QUALITIES = ["144p", "240p", "360p", "480p", "720p", "1080p", "128kbps"];
...
const getDownloadLink = async id => {
  ...
  try {
    ...
    if (!response.data || !response.data.links) throw new Error("Tidak ada respons dari API.");
    return Object.entries(response.data.links).reduce((acc, [format, links]) => {
      acc[format] = Object.fromEntries(Object.values(links).map(option => [option.q || option.f, async () => {
        const res = await axios.post("https://id-y2mate.com/mates/convertV2/index", new URLSearchParams({
          vid: id,
          k: option.k
        }), {
          headers: headers
        });
        if (res.data.status !== "ok") throw new Error("Error saat konversi.");
        return {
          size: option.size,
          format: option.f,
          url: res.data.dlink
...
const cleanInput = (input, command) => {
  const urlMatch = input.match(/(https?:\/\/(?:www\.)?youtu\.be\/[^\s]+|https?:\/\/(?:www\.)?youtube\.com\/[^\s]+)/i);
  const qualityMatch = input.match(/--q=([a-zA-Z0-9]+p|kbps)$/i);
  const url = urlMatch ? urlMatch[0] : null;
  const quality = qualityMatch ? qualityMatch[1] : command === "y2matemp3" ? DEFAULT_QUALITY_MP3 : DEFAULT_QUALITY_MP4;
  return {
    url: url,
    quality: quality
  };
...
const handler = async (m, {
  ...
  try {
    ...
    if (!input) {
      return m.reply(`Masukkan URL video YouTube atau query pencarian.\nContoh:\n*${usedPrefix}${command}* https://youtube.com/watch?v=YQHsXMglC9A --q=720p`);
    }
    await m.react(wait);
    const {
      videoData,
      quality
    } = await getYoutubeVideoData(input, command);
...
`;
    const infoReply = {
      contextInfo: {
        externalAdReply: {
          body: `Sedang mendownload ${command.toUpperCase()}, harap tunggu...`,
          mediaType: 1,
...
handler.help = ["y2matemp3", "y2matemp4"].map(v => `${v} <url> [--q=quality]`);
