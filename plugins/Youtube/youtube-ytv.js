...
const handler = async (m, {
  ...
  try {
    ...
    await conn.reply(m.chat, `*\`W A I T\`*\n🔗 *Link:* ${vid.url}`, m, infoReply);
    let data = null;
    const sources = [invidious, notube, cobalt, quick];
    for (const source of sources) {
      try {
        data = source === invidious ? await source.detail(getId(vid.url), isMP3 ? "mp3" : "mp4") : await source(vid.url, isMP3 ? "mp3" : "mp4");
    ...
    infoReply.contextInfo.externalAdReply.thumbnailUrl = data.thumbnail || vid.thumbnail;
    const captvid = `${data.title || vid.title ? `📺 *Judul:* ${data.title || vid.title}\n` : ""}
...
${data.description || data.detail?.description ? `📜 *Deskripsi:* ${data.description || data.detail.description}` : ""}`.trim();
    await conn.sendMessage(m.chat, {
      ...isDOC ? {
        document: is8k ? await (await toAudio8k(data.buffer, "mp3"))?.toBuffer() : data.buffer,
        mimetype: isMP3 ? "audio/mp4" : "video/mp4",
        caption: captvid,
        fileName: data.title || vid.title || getId(vid.url)
      } : isMP3 ? {
        audio: is8k ? await (await toAudio8k(data.buffer, "mp3"))?.toBuffer() : data.buffer,
        mimetype: "audio/mp4"
      } : {
        video: is8k ? await (await toAudio8k(data.buffer, "mp3"))?.toBuffer() : data.buffer,
        mimetype: "video/mp4",
        caption: captvid
      },
...
handler.help = ["mp4", "v", "vdl"].map(v => "yt" + v + " <url>");
