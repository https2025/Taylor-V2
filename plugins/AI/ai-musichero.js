    ...
    const infoReply = {
      contextInfo: {
        ...
        externalAdReply: {
          ...
          thumbnailUrl: answer.image,
          sourceUrl: ""
        }
      }
    };
    await conn.reply(m.chat, `*\`MusicHero\`*\n🔗 *Model:* ${answer.model}`, m, infoReply);
    const {
      key: {
        id: keyId
      }
...
handler.before = async (m, {
  ...
  if (!db.data.dbai.musichero || m.isBaileys || !(m.sender in db.data.dbai.musichero)) return;
  const {
    key: {
      id: keyId
    }
