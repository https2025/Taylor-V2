...
class Lalals {
  ...
  async createMusic(input_text, input_tags) {
    ...
    const arr = ["26088041-fa27-4e24-921c-e21603c49c04", "4ffc412e-94be-4abe-bd37-a64db39e6ca1", "66cf9ae4-09cf-4df0-b65a-300191bb5aec", "ab186086-1428-496a-bbae-215612120ddd", "bd085997-6de5-402a-8f3e-70805e3d8b91", "4c68a378-d17d-4065-b9f8-4749c89a7e96", "967bc7ff-abec-444a-bca1-327ec5179a43", "c1a6dda5-05f6-4c0d-9686-57e146ded467", "6f0de919-f896-49ee-835f-fb945756b357", "6d7a58e3-e69c-4452-a321-cb10b768e004", "40fab8f7-8555-48a7-be96-e357237296c9", "4025bd17-0aa2-4af3-8e67-4d2531589dea", "adbbad1c-c976-4b68-a71a-587852e67ea0", "07250062-3f9f-4a6c-ba2d-0038bd3d6e3b", "53348a97-ff9a-4003-b722-aad2aafe28c8"];
    ...
    try {
      ...
      if (task_id) {
        ...
        let interval = 3e3;
        while (true) {
          const workListResponse = await this.request("https://devapi.lalals.com/user/e79764c3-90b7-4a4e-8e8f-7e4428443fd6/projects?offset=0", "GET", null, headers);
          const {
            rows: workList
          } = workListResponse;
          ...
          if (work) {
            ...
            console.log("Timeout: UID tidak ditemukan di work list setelah 1 menit");
            return null;
          } else {
            await new Promise(resolve => setTimeout(resolve, interval));
          }
...
const handler = async (m, {
  ...
  let inputText = args.length ? args.join(" ") : m.quoted?.text || m.quoted?.caption || m.quoted?.description || null;
  if (!inputText) {
  ...
  const inputTextClean = styleMatch ? inputText.replace(/--style=[^]+$/, "").trim() : inputText.trim();
  m.react(wait);
  ...
  try {
    ...
    const infoReply = {
      contextInfo: {
        ...
        externalAdReply: {
          title: `Lalals AI`,
          body: `Mengunduh audio harap tunggu...`,
          mediaType: 1,
          previewType: 0,
          renderLargerThumbnail: true,
          thumbnailUrl: answer.voices[0].profile_picture || thumb,
          sourceUrl: answer.track_url || answer.audio_link,
          mediaUrl: answer.track_url || answer.audio_link
        }
      }
    };
    await conn.reply(m.chat, `*\`LalalsAi\`*\n*Voice:* ${answer.voices[0].voice_name}\n🔗 *Audio:* ${answer.audio_link}`, m, infoReply);
    const {
      key: {
        id: keyId
      }
...
handler.before = async (m, {
  ...
  if (!db.data.dbai.lalals || m.isBaileys || !(m.sender in db.data.dbai.lalals)) return;
  const {
    key: {
      id: keyId
    }
  ...
  if (m.quoted?.id === keyId && m.text.trim()) {
    ...
    const style = styleMatch ? styleMatch[1].trim() : null;
    const inputTextClean = styleMatch ? m.text.trim().replace(/--style=[^]+$/, "").trim() : m.text.trim();
    m.react(wait);
    const lalals = new Lalals();
    try {
      const answer = await lalals.createMusic(inputTextClean, style);
