...
class SunoAi {
  constructor() {
    this.apiKey = pickRandom(["VCwrNNJ1msu3dOQmGr46AM3WLxoecqLl", "bw0f/AFAdYQ3QVX3ZkM9ZrnncYH/iCRl"]);
...
const handler = async (m, {
  ...
  const inputTextClean = styleMatch?.[1]?.trim() ? inputText.replace(/--style=[^]+$/, "")?.trim() : inputText.trim();
  m.react(wait);
  try {
    ...
    const infoReply = {
      contextInfo: {
        ...
        externalAdReply: {
          ...
          thumbnailUrl: result.image_large_url,
          sourceUrl: ""
        }
      }
    };
    await conn.reply(m.chat, `*\`SunoAi\`*\n🔗 *Model:* ${result.model_name}\n🎵 *Judul:* ${result.title}\n📅 *Dibuat:* ${result.created_at}\n🎶 *Durasi:* ${result.meta_data.duration} detik\n🎧 *Tags:* ${result.meta_data.tags}`, m, infoReply);
    const {
      key: {
        id: keyId
      }
...
handler.before = async (m, {
  ...
  if (!db.data.dbai.SunoAi || m.isBaileys || !(m.sender in db.data.dbai.SunoAi)) return;
  const {
    key: {
      id: keyId
    }
  ...
  if (m.quoted?.id === keyId && m.text.trim()) {
    ...
    const style = styleMatch?.[1]?.trim() ? styleMatch[1].trim() : null;
    const inputTextClean = styleMatch?.[1]?.trim() ? m.text.trim().replace(/--style=[^]+$/, "")?.trim() : m.text.trim();
    m.react(wait);
    try {
      const sunoAi = new SunoAi();
      const answer = await sunoAi.generateMusic({
