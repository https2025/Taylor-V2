...
const handler = async (m, {
  ...
  if (db.data.game.akinator = db.data.game.akinator || {}, "end" === text) {
    if (!db.data.game.akinator[m.sender]) return m.reply("Anda tidak sedang dalam sesi Akinator");
    delete db.data.game.akinator[m.sender], m.reply("Berhasil keluar dari sesi Akinator.");
  } else if ("start" === text) {
    if (db.data.game.akinator[m.sender]) return await conn.reply(m.chat, "Anda masih berada dalam sesi Akinator", db.data.game.akinator[m.sender].msg);
    try {
      ...
      let soal = await conn.sendMessage(m.chat, {
        ...
        externalAdReply: {
          ...
          thumbnailUrl: "https://id.akinator.com/assets/img/akinator.png",
          sourceUrl: "",
          mediaUrl: ""
        }
      }
