...
const handler = async (m, {
  ...
  try {
    ...
    if (jid === m.sender) {
      return conn.reply(m.chat, "Tidak bisa mengirim pesan menfess ke diri sendiri.", m);
    }
    const activeMenfess = Object.values(database).some(mf => mf.status === true);
    ...
    await conn.reply(m.chat, `*Berhasil mengirim pesan ke @${jid.replace(/@.+/, "")}*\n\n${logs}`, m, {
      contextInfo: {
        externalAdReply: {
          showAdAttribution: true,
          mediaUrl: sfb,
    ...
    console.error(err);
    return conn.reply(m.chat, "Terjadi kesalahan saat mengirim pesan. Coba lagi nanti.", m);
  }
};
handler.tags = ["main"];
