  ...
  try {
    ...
    for (let i = 0; i < 5; i++) {
      try {
        ...
        slides.push([title, footer, header, images_url, [
    ...
    await conn.sendSlide(m.chat, `Pinterest Results`, `Downloaded from ${text}`, slides, m);
    m.react(sukses);
  } catch (e) {
...
handler.help = handler.alias = ["pinterest"], handler.tags = ["image"], handler.command = /^(pin|pinterest)$/i;
...
async function pinterest(query) {
  try {
    if (query.match(URL_REGEX)) {
      ...
      const data = cheerio.load(await res.text())('table[class="table table-condensed table-striped table-bordered"]').find("a").attr("href");
