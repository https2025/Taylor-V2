...
export async function handler(chatUpdate) {
  ...
  await this.pushMessage(chatUpdate.messages).catch(e => console.log(e));
  ...
  if (!db.data && db.data === null) await loadDatabase();
  try {
    m = smsg(this, m) || m;
    if (!m) return;
    m.exp = 0;
    ...
    try {
      ...
      let rpg = db.data?.rpg;
      if (typeof rpg !== "object") db.data.rpg = {};
      let game = db.data?.game;
      if (typeof game !== "object") db.data.game = {};
      let dbbot = db.data?.dbbot;
      if (typeof dbbot !== "object") db.data.dbbot = {};
    } catch (e) {
      console.error(e);
    }
    ...
    const groupMetadata = (m.isGroup ? (this.chats[m.chat] || {}).metadata || await this.groupMetadata(m.chat).catch(e => console.log(e)) : {}) || {};
...
export async function participantsUpdate({
  ...
  switch (action) {
    ...
    case "remove":
      if (chat.welcome) {
        const groupMetadata = await (conn || this).groupMetadata(id).catch(e => console.log(e)) || ((conn || this).chats[id] || {}).metadata || {};
        for (let user of participants) {
          user = user || id || (conn || this).user.jid;
          const isAddAction = action === "add";
          const welcomeText = isAddAction ? (chat.sWelcome || (conn || this).welcome || `${emoji.welcome} Selamat datang, @user!`).replace("@subject", groupMetadata.subject?.toString()).replace("@desc", groupMetadata.desc?.toString() || "tidak diketahui") : chat.sBye || (conn || this).bye || `${emoji.bye} Sampai jumpa, @user!`;
          const editor = new ImageEditor(thumb, await (conn || this).profilePictureUrl(user, "image").catch(() => hwaifu.getRandom()));
          const welAp = await Welcome(await (conn || this).profilePictureUrl(user, "image").catch(() => hwaifu.getRandom()), groupMetadata.subject?.toString(), await (conn || this).getName(user), parseInt(participants.length), "Welcome to our group!", welcomeText.split(" ").slice(0, 10).join(" "), parseInt(id), thumb);
          const ieditwBuff = decodeURIComponent(welAp) || await editor.asyncWelcome(await (conn || this).getName(user), "Welcome to our group!", welcomeText) || (await (conn || this).getFile(thumb))?.data;
          const leaAp = await Leave(await (conn || this).profilePictureUrl(user, "image").catch(() => hwaifu.getRandom()), groupMetadata.subject?.toString(), await (conn || this).getName(user), parseInt(participants.length), "Leave in group!", welcomeText.split(" ").slice(0, 10).join(" "), parseInt(id), thumb);
          const ieditlBuff = decodeURIComponent(leaAp) || await editor.asyncLeave(await (conn || this).getName(user), "Leave in group!", welcomeText) || (await (conn || this).getFile(thumb))?.data;
          const spaces = "                    ";
  ...
  for (const {
    ...
    if (message.pollUpdates) {
      ...
      if (pollCreation) {
        ...
        this.appendTextMessage(message, message.pollUpdates[0]?.vote || pollMessage.filter(v => v.voters.length !== 0)[0]?.name, message.message);
      }
    }
  }
}
...
export async function presenceUpdate(presenceUpdate) {
  ...
  if (user?.afk && status === "composing" && user.afk > -1) {
    ...
    db.data.database.afk[id] = db.data.database.afk[id] ? db.data.database.afk[id].filter(user => user.id !== idToRemove) : [];
