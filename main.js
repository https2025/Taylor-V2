...
global.loadDatabase = async function loadDatabase() {
  ...
  }) : db.data === null ? (db.READ = true, await db.read()?.catch(e => console.log(e)), db.READ = null, db.data = {
    users: {},
    chats: {},
    stats: {},
    msgs: {},
...
const connectionOptions = {
  ...
  browser: useQr ? ["Taylor-V2", "Safari", "2.0.0"] : ["Ubuntu", "Chrome", "20.0.04"],
...
global.reloadHandler = async function reloadHandler(restatConn) {
  try {
    const Handler = await (await import(`./handler.js?updated=${Date.now()}`))?.catch(e => console.log(e));
...
async function runTasks() {
  ...
  const results = [];
  try {
    await Promise.all(tasks.filter(t => t.run).map(async task => {
      try {
        spinner.start(chalk.bgYellow.black(`Running task: ${task.message}\n`));
        await task.func();
        spinner.succeed(chalk.bgYellow.black(`${task.message} completed\n`));
        results.push({
          status: "fulfilled",
...
async function loadLibs() {
  ...
  const importPromises = lodash.map(LibsFiles, async file => {
    const moduleName = path.join("/lib", path.relative(path.resolve(directoryName, "lib"), file));
    try {
      const fileContent = readFileSync(file, "utf-8");
      const err = syntaxerror(fileContent, file, {
        sourceType: "module",
      ...
      if (err) {
        ...
        const module = await import(`${file}`);
        setNestedObject(lib, moduleName.slice(0, -3), module.default ? module.default : module);
        return {
          moduleName: moduleName,
          filePath: file,
          success: true
...
const setNestedObject = (obj, filePath, value) => filePath.split("/").reduce((acc, key, index, keys) => index === keys.length - 1 ? acc[key] = value : acc[key] = acc[key] || {}, obj);
async function watchFiles() {
  try {
    const mainDir = path.resolve(directoryName, "");
    console.log(`Main Dir: ${mainDir}`);
    const watcher = chokidar.watch(mainDir, {
      ignored: (filePath, stats) => stats?.isFile() && (filePath.match(/node_modules/) || filePath.startsWith(".") || !filePath.endsWith(".js")),
      persistent: true,
      ignoreInitial: true,
      alwaysState: true
    });
    watcher.on("add", async filePath => {
      ...
      try {
        const dir = filePath.match(/plugins/) ? "plugins" : filePath.match(/lib/) ? "lib" : "main";
        const resolvedFile = path.join(`/${dir}`, path.relative(path.dirname(path.dirname(filePath)), path.dirname(filePath)), path.basename(filePath));
        ...
        if (dir === "plugins") {
          ...
          setNestedObject(lib, resolvedFile.slice(0, -3), module.default ? module.default : module);
          ...
          console.warn(`File added outside of recognized directories: ${filePath}`);
          logger.warn(`File added outside of recognized directories: ${filePath}`);
        }
      } catch (e) {
      ...
      try {
        ...
        if (dir === "plugins") {
          ...
          console.warn(`File changed outside of recognized directories: ${filePath}`);
          logger.warn(`File changed outside of recognized directories: ${filePath}`);
        }
      } catch (e) {
        console.error(`Error handling 'change' event for '${filePath}': ${e}`);
      ...
      try {
        ...
        if (dir === "plugins") {
          ...
          console.warn(`File deleted outside of recognized directories: ${filePath}`);
          logger.warn(`File deleted outside of recognized directories: ${filePath}`);
        }
      } catch (e) {
        console.error(`Error handling 'unlink' event for '${filePath}': ${e}`);
