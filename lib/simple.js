    ...
    sendFile: {
      async value(jid, path, filename = "", caption = "", quoted, ptt = false, options = {}) {
        try {
          ...
          const base64File = file instanceof Uint8Array ? file : Buffer.isBuffer(file) ? Uint8Array.from(Buffer.from(file)) : typeof file === "string" ? Buffer.from(file, "base64") ? Uint8Array.from(Buffer.from(file, "base64")) : new Uint8Array() : new Uint8Array();
    ...
    sendContact: {
      async value(jid, data, quoted, options) {
        ...
        for (let [number, name] of data) {
          ...
          let biz = await conn.getBusinessProfile(njid).catch(e => console.log(e)) || {};
          let vcard = `
BEGIN:VCARD
VERSION:3.0
FN:${name.replace(/\n/g, "\\n")}
...
END:VCARD`.trim();
    ...
    processMessageStubType: {
      async value(m) {
        ...
        chats.isChats = true;
        const metadata = await conn.groupMetadata(chat).catch(e => console.log(e));
        if (!metadata) return;
    ...
    pushMessage: {
      async value(m) {
        ...
        for (const message of m) {
          try {
            if (!message) continue;
            if (message.messageStubType && message.messageStubType != WAMessageStubType.CIPHERTEXT) conn.processMessageStubType(message).catch(e => console.log(e));
            ...
            let chats = conn.chats[chat];
            if (!chats) {
              if (isGroup) await conn.insertAllGroup().catch(e => console.log(e));
            ...
            if (isGroup) {
              if (!chats.subject || !chats.metadata) {
                metadata = await conn.groupMetadata(chat).catch(e => console.log(e)) || {};
