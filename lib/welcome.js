...
const apis = {
  welcome: ["https://widipe.com/welcome", "https://widipe.com/welcome2", "https://widipe.com/welcome3", `https://some-random-api.com/welcome/img/${Math.floor(7 * Math.random()) + 1}/stars`],
  leave: ["https://widipe.com/goodbye", `https://some-random-api.com/welcome/img/${Math.floor(7 * Math.random()) + 1}/stars`]
};
...
function createParams(url, {
  ...
  const removeKeys = {
    "widipe.com": ["avatar", "username", "discriminator", "guildName", "memberCount", "textcolor", "key", "font"],
        ...
        case "widipe.com":
          return {
            ...
            pp: encodeURIComponent(profile || defaultProfile()),
              ppgc: encodeURIComponent(bg || defaultBG()),
              gcname: encodeURIComponent(gcname || "Group"),
...
async function generateUrl(apiType, index, Profile, GC, Name, Count, Text, WL, uid, bg) {
  const urls = apis[apiType];
  const url = urls[index >= 0 && index < urls.length ? index : Math.floor(Math.random() * urls.length)];
  const profileUrl = Profile || defaultProfile();
  const bgUrl = bg || defaultBG();
  const resolvedProfileUrl = url.includes("some-random-api") ? await fetchLocation(profileUrl) : profileUrl;
  const resolvedBgUrl = url.includes("some-random-api") ? await fetchLocation(bgUrl) : bgUrl;
...
const Welcome = (Profile, GC, Name, Count, Text, WL, uid, bg, index) => generateUrl("welcome", index, Profile, GC, Name, Count, Text, WL, uid, bg);
const Leave = (Profile, GC, Name, Count, Text, WL, uid, bg, index) => generateUrl("leave", index, Profile, GC, Name, Count, Text, WL, uid, bg);
